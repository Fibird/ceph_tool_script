...
#                echo "[INFO]: PID:$!: rados bench writing to "$pool_name"..."
            ...
            iops_sum=0;  res_sum=0;  wgt_sum=0;  lim_sum=0;  rem_iops=0
            declare -a iopses
            ...
            for ((i=0;i<$pool_num;i++)); do
                ...
                result_pre="$qos_id,$pool_name,$r_value,$w_value,$l_value,$iops"
            ...
            for ((i=0;i<$pool_num;i++)); do
                ...
                iops=$(grep "write: IOPS=" $result_file_name | awk '{print $2}' | tr -d  'IOPS=,')
                if [[ $rem_iops -gt 0 ]]; then
                    real_iops=$((r_list[$i]+rem_iops*w_list[$i]/wgt_sum))
                    if [[ ${l_list[$i]} -ne 0 && $real_iops -gt ${l_list[$i]} ]]; then
                        real_iops=${l_list[$i]}
                        bonus_iops=$((bonus_iops+real_iops-l_list[$i]))
                    else
                        ((rem_wgt+=w_list[$i]))
                    fi
                fi
                ept_iopses[$i]=$real_iops
            done
